...
<body class="font-sans antialiased">
    ...
    .then(() => {
        // Immediately update the UI if the dropdown is open
        const panel = document.getElementById('favoritesDropdown');
        if (!panel.classList.contains('d-none')) {
            loadFavorites(); // ✅ Re-render list
        }

        // Now sync the favorites and update the cache
        return fetch('/favorites');
    })
    .then(res => res.json())
    .then(data => {
        const updatedFavorites = data.map(fav => fav.place_id);
        savedFavorites = updatedFavorites;

        const cached = localStorage.getItem('mosqueCache');
        if (cached) {
            const parsed = JSON.parse(cached);
            parsed.savedFavorites = updatedFavorites;
            localStorage.setItem('mosqueCache', JSON.stringify(parsed));
        }
    })
    .catch(() => {
        console.error('Failed to remove favorite or sync cache.');
    });
}
